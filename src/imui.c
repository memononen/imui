...
// Layout code based on https://github.com/icedman/ccx
...
*/
		...
		if (strcmp(key, "font") == 0) {
			...
			imui__addPropFloat(style, state, IMUI_PROP_PADDING_LEFT, padding[3]);
...
/*		} else if (strcmp(key, "margin") == 0) {
			...
			imui__addPropFloat(style, state, IMUI_PROP_MARGIN_LEFT, margin[3]);
			...
			if (nvals == 1) {
				...
				imui__addPropFloat(style, state, IMUI_PROP_BOXSHADOW_SPREAD, imui__parseSizePx(p, vals[3]));
				imui__addPropUint(style, state, IMUI_PROP_BOXSHADOW_COLOR, imui__parseColor(p, vals[4]));
			}
...
static void imui__calculateSizes(struct IMUIbox* box, int depth)
{
	struct IMUIbox* it;
	float ms, cs, percent;
...
//	printf("<%s> comp=(%.1f, %.1f) cont=(%.1f, %.1f)\n", box->type, box->computedSize.x,box->computedSize.y, box->contentSize.x,box->contentSize.y);
...
//	printf("%*scalcsize <%s> req=(%.1f, %.1f) elem=(%.1f, %.1f) cont=(%.1f, %.1f) comp=(%.1f, %.1f) cs=%f ms=%f\n", depth*2,"", box->type, box->requestedSize.x,box->requestedSize.y, box->elementSize.x,box->elementSize.y, box->contentSize.x,box->contentSize.y, box->computedSize.x,box->computedSize.y, cs, ms);
	...
	// Fit box to content.
//	printf("%*s    <%s> ms=%f cs=%f box: ms=%f cs=%f \n", depth*2,"", box->type, ms,cs, box->mainSize, box->crossSize);

	ms = imui__maxf(ms, box->mainSize - imui__calcPadding(box, box->dir));
	cs = imui__maxf(cs, box->crossSize - imui__calcPadding(box, imui__crossDir(box->dir)));

	// Auto matches content
	if ((int)box->requestedSize.x == IMUI_AUTO) {
//		printf("%*s%s auto X (%f %f)\n", depth*2, "",box->type, cs, ms);
		if (box->dir == IMUI_COL)
			box->computedSize.x = cs;
		else
			box->computedSize.x = ms;
//		box->computedSize.x += imui__calcPadding(box, IMUI_ROW);
		box->requestedSize.x = box->computedSize.x;
	}
	if ((int)box->requestedSize.y == IMUI_AUTO) {
//		printf("%*s%s auto Y (%f %f)\n", depth*2,"", box->type, cs, ms);

/*		for (it = box->items; it != NULL; it = it->next)
			printf("%f ",it->crossSize);
		printf("\n");*/
		if (box->dir == IMUI_COL)
			box->computedSize.y = ms;
		else
			box->computedSize.y = cs;
//		box->computedSize.y += imui__calcPadding(box, IMUI_COL);
		box->requestedSize.y = box->computedSize.y;
	}

//	printf("   <%s> comp=(%.1f, %.1f) cont=(%.1f, %.1f)\n", box->type, box->computedSize.x,box->computedSize.y, box->contentSize.x,box->contentSize.y);

	// Handle "stretch"
	cs = imui__maxf(1.0f, cs - imui__calcPadding(box, imui__crossDir(box->dir)));
	for (it = box->items; it != NULL; it = it->next) {
...
static void imui__growItems(struct IMUIbox* box, int depth)
{
	struct IMUIbox* it;
...
static void imui__packItems(struct IMUIbox* box, int depth)
{
	struct IMUIbox* it;
...
//	mainSize = imui__maxf(1.0f, box->mainSize - imui__calcPadding(box, box->dir));
	mainSize = imui__maxf(1.0f, box->mainSize);
...
//		printf("%*s   <%s> (%.1f,%.1f) %f~(%.1f) sp=%f\n", it->type, it->computedPosition.x, it->computedPosition.y, it->mainSize, it->contentSize.x, spacing);
		ms += it->mainSize;
		ms += spacing;
		imui__packItems(it, depth+1);
	}
	}
}

static void imui__alignItems(struct IMUIbox* box, int depth)
{
	struct IMUIbox* it;
	float crossSize;
...
//	crossSize = box->crossSize;
	...
	for (it = box->items; it != NULL; it = it->next) {
		...
		char align;
		imui__updateMetrics(it, box->dir);
		availableCrossSpace = crossSize - it->crossSize; //imui__maxf(0.0f, crossSize - it->crossSize);
		align = box->align;
		// check for self alignment
		if (it->alignSelf != IMUI_AUTO)
			align = it->alignSelf;
...
/*	printf("%*s- %s: con=(%f x %f) req=(%f x %f) pad=(%f %f %f %f)\n",
		depth*2, "", box->type,
		box->contentSize.x,box->contentSize.y, box->requestedSize.x,box->requestedSize.y,
		box->computedStyle.padding[0],box->computedStyle.padding[1],box->computedStyle.padding[2],box->computedStyle.padding[3]);*/
...
//	printf("Sizes (%f,%f):\n", w, h);
	...
	imui__calculateSizes(tmpl->root, 0);
	imui__growItems(tmpl->root, 0);
	imui__packItems(tmpl->root, 0);
	imui__alignItems(tmpl->root, 0);
